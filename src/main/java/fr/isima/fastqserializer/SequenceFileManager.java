...
public class SequenceFileManager {
	 ...
	 * 
	 * @param sc:
	 *            SparkContext in which we're operating
	 * @param filePath:
	 *            path oto the file we would like to convert (.fq)
	 * @param resultFolder:
	...
	public void convertFastqToFqrdd(JavaSparkContext sc, String filePath, String resultFolder) throws IOException {
		...
		/*
		 * JavaPairRDD<FastxRecord,String> fqrdd = fastqRDD.mapToPair( new
		 * PairFunction<FastxRecord, FastxRecord, String>(){ public
		 * Tuple2<FastxRecord,String> call(FastxRecord fq){
		 * 
		 * return new Tuple2<FastxRecord,String>(fq,""); } }); //
		 */

		// fqrdd.saveAsHadoopFile(resultFolder, FastxRecord.class, String.class,
	 ...
	 * makes a JavaRDD of containing objects representing the data read those
	 * files are the result of a previous Spark operation name format: part#####
	 * ex: part-00000 the folder must also contain an empty file with the name
	 * _SUCCESS
	 * 
	 * @param sc:
	 *            Spark Context in which we're operating
	 * @param folderPath:
	 *            path to the folder which we want to read
	...
	/**
	 * Reads a FqRdd folder and then filters it according to the criterias
	 * specified
	 * 
	 * @param sc:
	 *            Spark Context in which we're operating
	 * @param folderPath:
	 *            Path to the fqrdd folder
	 * @param minLength:
	...
	// with ID: il faudrait qu'une fonction qui gère with id or not (si c'est
	// NULL or pas)
	...
	public void getKmerKmeansClustering(JavaSparkContext sc, String folderPath, int order, int nbClusters,
