...
public class SequenceFileInterrogator implements Serializable {
	...
	public void getFqRDDSatistics(JavaRDD<FastqRecord> fqrdd) throws IOException {
		...
		JavaPairRDD<FastqRecord, Integer> meanSequencesQualities = fqrdd
				.mapToPair(new PairFunction<FastqRecord, FastqRecord, Integer>() {
					public Tuple2<FastqRecord, Integer> call(FastqRecord r) {
						return new Tuple2<FastqRecord, Integer>(r, r.getMeanQuality()); // Recupération
																						// des
																						// séquences
					}
				});
		...
		// REDUCE
		JavaPairRDD<Integer, Integer> distLenght = distributions
		 ...
		 * System.out.println("------------------------------");
		 * JavaRDD<FastqRecord> meanQual = fqrdd; int pos = 0; long nb =
		 * meanQual.count(); // on devrait utilisere nbEntries
		 * 
		 * boolean cont = !meanQual.isEmpty(); for(;cont == true ; ){
		 * 
		 * JavaPairRDD<Integer,Integer> res= meanQualityInPos(meanQual, pos);
		 * 
		 * JavaPairRDD<Integer, Integer> quality = res.reduceByKey(new
		 * Function2<Integer, Integer, Integer>() { public Integer call(Integer
		 * a, Integer b) { return a + b; } });
		 * 
		 * System.out.println( quality.first()._1 + " -> " + quality.first()._2
		 * + " / " + nb); System.out.println( quality.first()._1 + " -> " +
		 * quality.first()._2 / nb);
		 * 
		 * //meanQual = filterJavaRdd(meanQual, pos, Integer.MAX_VALUE, 0,
		 * Integer.MAX_VALUE,true); nb = meanQual.count(); ++pos; cont =
		 * !meanQual.isEmpty(); //cont = nb > pos;
		 * 
		 * } //
		 */
	}
	...
	/*
	 * Fonction qui retourne la qualité moyenne pour une position parmis un
